...
```{r mapping occurences}
mapr::map_ggplot(list_longlat_amphibian)
mapr::map_ggplot(out_reptile$gbif$data$Alligator_mississippiensis)
...
```{r furrr::future_map for speedy extract}
...
tic("extract_furrr")
extract_spatial_mammal_lapply_furrr <- furrr::future_map(.x = list_spatial_mammal, .f = extracting_stackbio)
toc()
```

But We didn't standardized them.  
...
paste(top4_pc1_mammal[[1]],".tif", sep = "")
...
model_aves_maxent <- map2(aves_bios, train_fold5_aves, maxent)
